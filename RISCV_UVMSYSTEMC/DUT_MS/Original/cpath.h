...
//            std::cout << "**** DUT **** request instruction addr: 0x" << hex << memoryAccess.addrIn << "\n";
            CtlToMem_port->write(memoryAccess);
            //Read decoded instruction from decoder
            MemToCtl_port->read(encodedInstr);
//            std::cout << "\t\t\t\t\t\t\t\t\t\t**** DUT **** receive instruction: " << hex << encodedInstr.loadedData << "\n";

            // Terminate if:
            // Zero instruction is read
#if SCAM == 0
            if (encodedInstr.loadedData == 0x100073) {
        ...
        if (section == Sections::memoryOperation){
            ...
            if (mem_en) {
                ...
                if (memoryAccess.req == ME_AccessType::ME_WR)
                    memoryAccess.dataIn = RegsToCtl_data.contents2;    // rs2 = source for store
                else if (memoryAccess.req == ME_AccessType::ME_RD)
                    memoryAccess.dataIn = 0; // FIXME: there was no assignment to the data before
                ...
                // Request load/store
//                std::cout << "**** DUT **** request MEM addr: 0x" << hex << memoryAccess.addrIn << ", data: "<< memoryAccess.dataIn << "\n";
                CtlToMem_port->write(memoryAccess);

                // Load/store done
                MemToCtl_port->read(fromMemoryData);
//                std::cout << "\t\t\t\t\t\t\t\t\t\t**** DUT **** receive MEM: " << hex << fromMemoryData.loadedData << "\n";
