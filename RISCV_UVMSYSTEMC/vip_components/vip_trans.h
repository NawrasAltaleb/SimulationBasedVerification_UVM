...
//    virtual void do_copy( const uvm_object* rhs ) { ... }
    virtual bool do_compare( const uvm_object& rhs ) const {
        const vip_trans_toMemory* rhs_ = dynamic_cast<const vip_trans_toMemory*>(&rhs);
        if(rhs_ == NULL)
            UVM_FATAL("do_compare", "cast failed, check type compatibility");

        return ((toMemory.addrIn == rhs_->toMemory.addrIn) &&
                (toMemory.dataIn == rhs_->toMemory.dataIn) &&
                (toMemory.req == rhs_->toMemory.req) &&
                (toMemory.mask == rhs_->toMemory.mask));
    }
}; // class vip_trans
