...
public:
    sequence_R <vip_trans_fromMemory, vip_trans_toMemory> *vip_seq;
...
//        std::cout << sc_core::sc_time_stamp() << ": constructor " << name << std::endl;
    ...
    void body() {
        UVM_INFO(get_name(), vip_seq->get_type_name(), uvm::UVM_NONE);
//        vip_seq->get_type_name();
//        UVM_DO_ON(vip_seq, p_sequencer->vip_dut_seqr); /// Macro to start sequence on a specific sequencer using its member function start
//        UVM_DO_ON(vip_seq, p_sequencer->vip_gm_seqr); /// Macro to start sequence on a specific sequencer using its member function start
    }
}; // class virt_sequence
