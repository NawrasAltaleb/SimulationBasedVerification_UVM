...
int sc_main(int, char **) {
    ...
    // Ports (Memory Interface)
    blocking_out<CUtoME_IF> COtoME_port("COtoME_port");
    blocking_in<MEtoCU_IF> MEtoCO_port("MEtoCO_port");

    // Channel instantiation
    MasterSlave<CUtoDP_IF> CUtoDP_channel("CUtoDP_channel");
    MasterSlave<DPtoCU_IF> DPtoCU_channel("DPtoCU_channel");

    MasterSlave<DPtoRF_IF> DPtoRF_channel("DPtoRF_channel");
    MasterSlave<RFtoDP_IF> RFtoDP_channel("RFtoDP_channel");
